...
// this code still need data structure optimization as it depends on arrays of fixed size resulting into space rigidity and linear time operations
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
int pass_validate(sign_in *users) // a method that returns an integer and takes a pointer of type struct (sign_in) as a parameter
{
    int i = 0; // counter declared

    char username[20];          // array of 20 characters for username
    printf("Enter username: "); // printf statement for prompt
    scanf("%s", username);      // scanf statement to take user input inside the username array

    char password[10];          // array of 20 characters to store password
    printf("Enter password: "); // printf statement for prompt
    scanf("%s", password);      // scanf to take user input and place it inside the password array

    for (i = 0; i < num_users; i++) // for loop to loop over all users
    {
        if (strcmp(username, users[i].username) == 0)     // conditional to check if the sting ig the username is equal to any username inside the array of users
        {                                                 // checking if it equals zero because strcmp is a case senstive method that returns zero if they are equal
            if (strcmp(password, users[i].password) == 0) // now if the username is found compare the password
            {
                printf("Authentication Successful.\n"); // printf to prompt the user for success
                return 1;                               // returning 1 to the caller to tell that excution was successful
            }
            else // else block to handle incorrect input case
            {
                printf("Incorrect password.\n");
                return 0; // returning 0 to tell the caller that excution wasn't successful
            }
        }
    }
    return 0; // return 0 for any unexpected behavior
}

/* Function to save account data to a file */
void save() // a method that returns nothing and takes nothing as a parameter
{
    int i;                           // counter
    f2 = fopen("accounts.txt", "w"); // direct file2 pointer to open the accounts.txt file in write mode

    if (f2 == NULL) // essential check if the file is not found
    {
        printf("File not in the system."); // prompt the user that there is an error
        return;                            // return without a value because the method returns nothing
    }

    if (counter_accounts == 0) // if the number of accounts equals zero return
        return;                // no curly brackets if only a single line is inside the if

    for (i = 0; i < counter_accounts; i++) // a loop to loop over the accounts and print them one by one inside the database file
    {
    ...
    fclose(f2); // close the file changes so it would be saved
    return;     // return to the caller to end method execution
}

/* Function to load account data from a file */
void load() // a method that takes no inputs and returns nothing
{
    f2 = fopen("accounts.txt", "r"); // direct file2 pointer to open the accounts.txt file in read only mode
    int i = 0;
    char accounts_arr[100]; // an array of characters to store inside it each account line by line for future tokenization and operations
    char delim2[] = ",-\n"; // declare the array of delimiters that will tokenize the string
    char *token2;           // an array of characters declared as a pointer to be a changing pointer for each word

    if (f2 == NULL) // check of the file is not found
    {
        printf("The file was not found");
            quit(); // if not found call the quit method to terminate program
    }
    counter_accounts = 0; // override the accounts to zero

    while (fgets(accounts_arr, 100, f2) != NULL) // a loop to loop over all txt file lines
    {                                            // each line will be retrieved using the fgets method and then added to the accounts
                                                 // character array and it will return null at the end declaring the end of the file

        token2 = strtok(accounts_arr, delim2); // strtok is a method that splits the string over the delimiters provided

        while (token2 != NULL) // while loop because the exact size is changing
        {

            strcpy(accounts[i].account_number, token2); // repeated operation of tokenizing and storing int the accounts array to every designated attribute inside the struct
        ...
        i++;
        counter_accounts++; // increase the number of accounts to know how many accounts exactly for future operations
    }
    fclose(f2); // close the file for changes to be saved
}

/* Function to convert month number to month name */
char *month_convert(int month_num) // this is a method that returns a pointer to a character (probably an array to store a string)
{                                  // it takes an integer as a parameter
    char *month = malloc(14);      // malloc to reserve a space of 14 bytes inside the heap for the month name

    switch (month_num) // switch case to switch over the month num
    {
    case 1:
    ...
    return month; // return the pointer to the caller for operations
}

/* Function to add a new account */
void add()
{
    int i, j;  // counters for outer and inner loops
    time_t t = time(0);
